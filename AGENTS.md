...
- `useJapanGeoJson()` from `@/lib/japan-geojson` loads and caches Season 19's generated Japan national outline (dissolved from the prefecture set for the outside-Japan mask). Use the hook instead of fetching the boundary independently; it returns `null` while loading.
- `useJapanPrefecturesGeoJson()` from `@/lib/japan-prefectures-geojson` loads and caches Season 19's generated Japan ADM1 prefecture boundaries (one feature per prefecture, with `name` and `shapeISO`). Use the hook instead of fetching the file independently; it returns `null` while loading.
- `resolveTeamLocations` and `resolvePrefectureUnlocks` from `@/seasons/season-19/game-board` project Season 19's Game Board tracker and unlock fills from the timeline. Pass both into `GameBoardCard`; use `resolveOccupiedOutlines(locations, unlocks)` for team-colored outlines on prefectures a team occupies without having unlocked.
