  ...
  return (
    <a className="border-paper/60 bg-panel hover:border-signal hover:shadow-card-hover focus-visible:border-signal focus-visible:shadow-card-hover group relative min-w-0 border transition duration-200 hover:-translate-y-1 focus-visible:-translate-y-1 focus-visible:outline-none" href={pageHref ? `/${pageHref}` : season.playlist} target={isAvailable ? undefined : "_blank"} rel={isAvailable ? undefined : "noreferrer"} aria-label={`Season ${season.number}: ${season.name}${isAvailable ? "" : " — coming soon"}`}>
      ...
      <div className="tablet:min-h-32 flex min-h-28 flex-col px-4.5 pt-4 pb-4">
        <h2 className="wide:text-lg max-w-9/10 text-base leading-tight font-bold tracking-tight text-balance">Season {season.number}: {season.name}</h2>
        <div className="mt-auto flex items-center justify-between"><span className="text-card-meta font-mono text-xs leading-none font-bold tracking-widest uppercase">{isAvailable ? "Explore season" : "Coming soon"}</span><i className="border-signal after:border-signal relative h-3 w-7 translate-y-1 border-t-2 after:absolute after:-top-1.5 after:right-0 after:size-2.5 after:rotate-45 after:border-t-2 after:border-r-2 after:content-['']" aria-hidden="true" /></div>
      </div>
    </a>
  );
}
  ...
  return (
    <main className="page-texture min-h-screen overflow-hidden">
      <section className="max-w-page px-gutter tablet:min-h-125 tablet:grid-cols-[1.2fr_.8fr] tablet:items-center tablet:pt-18 wide:min-h-140 wide:grid-cols-[1.04fr_.96fr] relative mx-auto grid min-h-130 grid-cols-1 items-start pt-12 pb-14" id="top">
        <div className="z-2">
          <h1 className="max-w-copy tablet:text-[clamp(72px,9vw,146px)] text-[clamp(64px,20vw,98px)] leading-[.84] font-black tracking-[-.045em] uppercase">Jet Lag:<br />The Player</h1>
        </div>
      ...
      <section className="max-w-page px-gutter tablet:pt-0 mx-auto pt-6 pb-18" aria-labelledby="archive-title">
        <div className="mb-6 flex items-center gap-4"><span className="before:bg-paper/60 after:bg-paper/60 relative size-5.5 shrink-0 before:absolute before:left-1/2 before:h-full before:w-px before:content-[''] after:absolute after:top-1/2 after:h-px after:w-full after:content-['']" aria-hidden="true" /><h2 className="font-display tablet:text-3xl text-2xl tracking-normal whitespace-nowrap uppercase" id="archive-title">Seasons</h2><span className="bg-paper/20 h-px flex-1" /></div>
        <div className="tablet:grid-cols-2 tablet:gap-6 wide:grid-cols-3 grid grid-cols-1 gap-5">{seasons.map((season) => <SeasonCard key={season.number} season={season} />)}</div>
      </section>
      ...
      <footer className="max-w-page border-paper/15 px-gutter tablet:h-64 relative mx-auto flex h-48 items-end justify-between overflow-hidden border-t py-10">
        <div className="border-paper/40 after:border-paper/25 relative z-1 grid size-28 -rotate-6 place-content-center rounded-full border text-center after:absolute after:inset-2 after:rounded-full after:border after:border-dashed after:content-['']"><span className="font-display text-4xl leading-none font-black">18½</span><small className="font-mono text-xs leading-tight font-bold tracking-widest uppercase">seasons<br />one world</small></div>
        <RouteMap className="top-auto right-[15%] bottom-[-28%] left-[15%] h-[120%] w-[70%] opacity-70" />
        <p className="text-footer-copy z-1 font-mono text-xs leading-none font-bold tracking-widest uppercase">Jet Lag: The Player</p>
      </footer>
    </main>
  );
}
