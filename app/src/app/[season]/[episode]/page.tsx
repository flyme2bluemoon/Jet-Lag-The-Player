    ...
    <main className="page-texture max-w-page px-gutter tablet:pt-7 tablet:pb-16 mx-auto min-h-screen w-full overflow-x-clip pt-4.5 pb-10">
      <header className="border-paper/20 tablet:flex-row tablet:items-end tablet:justify-between tablet:gap-8 tablet:pt-3 tablet:pb-4 flex flex-col items-stretch gap-6 border-b pt-7 pb-5.5">
        ...
        <nav className="border-paper/40 bg-panel tablet:inline-flex tablet:w-auto grid min-h-11 w-full grid-cols-[1fr_1.15fr_1fr] items-stretch overflow-hidden rounded-lg border" aria-label="Episode controls">
          {previousEpisode ? <Button asChild variant="ghost" className="text-control hover:text-paper focus-visible:text-paper hover:bg-paper/5 focus-visible:bg-paper/5 tablet:min-w-40 tablet:px-4 tablet:text-base h-auto min-w-0 gap-2 rounded-none border-0 bg-transparent px-2 font-display text-sm leading-none font-bold uppercase shadow-none"><Link href={episodeHref(previousEpisode.slug)}><Rewind className="size-4 shrink-0" aria-hidden="true" /> Previous episode</Link></Button> : <Button variant="ghost" className="bg-player/15 tablet:min-w-40 tablet:px-4 tablet:text-base h-auto min-w-0 gap-2 rounded-none border-0 px-2 font-display text-sm leading-none font-bold text-paper/30 uppercase shadow-none disabled:cursor-not-allowed disabled:opacity-100" disabled><Rewind className="size-4 shrink-0" aria-hidden="true" /> Previous episode</Button>}
          <EpisodeSwitcher seasonSlug={season.slug} episodes={season.episodes} currentSlug={episode.slug} />
          {nextEpisode ? <Button asChild variant="ghost" className="text-control hover:text-paper focus-visible:text-paper hover:bg-paper/5 focus-visible:bg-paper/5 tablet:min-w-40 tablet:px-4 tablet:text-base h-auto min-w-0 gap-2 rounded-none border-0 bg-transparent px-2 font-display text-sm leading-none font-bold uppercase shadow-none"><Link href={episodeHref(nextEpisode.slug)}>Next episode <FastForward className="size-4 shrink-0" aria-hidden="true" /></Link></Button> : <Button variant="ghost" className="bg-player/15 tablet:min-w-40 tablet:px-4 tablet:text-base h-auto min-w-0 gap-2 rounded-none border-0 px-2 font-display text-sm leading-none font-bold text-paper/30 uppercase shadow-none disabled:cursor-not-allowed disabled:opacity-100" disabled>Next episode <FastForward className="size-4 shrink-0" aria-hidden="true" /></Button>}
        </nav>
      </header>
      <EpisodeDashboard
