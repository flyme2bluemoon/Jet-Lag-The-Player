...
- Use `DashboardGrid` from `@/components/episode/dashboard-grid` for responsive dashboard layouts. Supply the `video`, `left`, `middle`, and `right` lanes rather than positioning columns in season code. It is single-column on small screens, uses a 7/5 video/sidebar split at `lg`, and becomes three columns at Tailwind's `2xl` viewport breakpoint (96rem/1536px), where the video spans the first two columns and every other lane remains one column wide. At the `ultrawide` breakpoint (125rem/2000px), it uses fixed 1/2/1 tracks: the `right` lane occupies the left column, the video occupies the middle column, and the `left` and `middle` lanes stack in the right column. Set the wide tracks with a stable, positive `wideColumnRatio` tuple ordered as `[left, middle, right]`; it defaults to `[1, 1.15, 1]` and does not affect the ultrawide ratio.
