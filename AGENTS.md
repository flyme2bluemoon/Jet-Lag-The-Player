...
- `useSwitzerlandGeoJson()` and `loadSwitzerlandGeoJson()` from `@/lib/switzerland-geojson` share one client-side fetch and parsed object for Season 9's `/geojson/switzerland.geojson`. Use the hook instead of importing or fetching the boundary independently; it returns `null` while loading. Publish changed geometry under a new filename because public GeoJSON assets are immutable-cached.
- `useGeoJson<T>(url)` and `loadGeoJson<T>(url)` from `@/lib/geojson` cache static GeoJSON assets by URL for the client session. Pass `null` to the hook to defer its request until the geometry is needed; it returns `null` while loading. Keep named dataset helpers when a shared asset benefits from a fixed type and URL.
