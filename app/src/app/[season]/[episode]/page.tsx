  ...
  return (
    <main className="page-texture max-w-page px-gutter tablet:pt-7 tablet:pb-16 2xl:max-w-none 2xl:px-8 mx-auto min-h-screen w-full overflow-x-clip pt-4.5 pb-10">
      <header className="border-paper/20 tablet:flex-row tablet:items-end tablet:justify-between tablet:gap-8 tablet:pt-3 tablet:pb-4 flex flex-col items-stretch gap-6 border-b pt-7 pb-5.5">
        <div>
          <Link className="text-copy-muted hover:text-paper focus-visible:text-paper tablet:mb-4 mb-6 inline-flex items-center gap-2 font-heading text-sm leading-none font-bold uppercase transition-colors focus-visible:outline-none" href={`/${season.slug}`}><span className="text-signal text-base" aria-hidden="true">←</span> {season.name}</Link>
          <p className="text-signal mb-2 font-display text-sm leading-none font-bold uppercase">Season {season.number}</p>
          <h1 className="font-heading leading-display-title tablet:text-[clamp(2.75rem,5vw,4.5rem)] text-4xl font-bold tracking-tight uppercase">{episode.label}</h1>
        </div>
